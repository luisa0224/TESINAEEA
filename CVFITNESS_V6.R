...
f1<-data %>% mutate(Sex=factor(RIAGENDR, labels = c("Female", "Male")))%>%
  ggplot(aes(y=CVDESVO2, fill=factor(stat_age), x=factor(stat_age)))+
  geom_boxplot()+
  theme_minimal()+
...
xrange<-range(data$CVDESVO2)
f3a<-ggplot(data, aes(x = CVDESVO2)) + 
  geom_histogram(aes(y = ..density..), binwidth = 1, color = "black", fill = "gray", alpha = 0.5) +
  geom_density(alpha = 0.5, color = "red", size = 1) +
  labs(title = expression(paste("VO"[2],"m·x antes de transformar")), x = "Valor", y = "Densidad") +
  geom_vline(xintercept=mean(data$CVDESVO2), linetype="dashed", color="red", size=1)+
  scale_x_continuous(limits = xrange, expand = c(0, 0)) +
  theme_minimal()

data$t_vo21<-orderNorm(data$CVDESVO2)$x.t
xrange<-range(data$t_vo21)
f3b<-ggplot(data, aes(x = t_vo21)) + 
  geom_histogram(aes(y = ..density..), binwidth = 0.2, color = "black", fill = "gray", alpha = 0.5) +
  geom_density(alpha = 0.5, color = "red", size = 1) +
  labs(title = expression(paste("VO"[2],"m·x despuÈs de transformar")), x = "Valor", y = "Densidad") +
  geom_vline(xintercept=mean(data$t_vo21), linetype="dashed", color="red", size=1)+
  scale_x_continuous(limits = xrange, expand = c(0, 0)) +
  theme_minimal()

fig3<-ggarrange(f3a, f3b, labels = LETTERS[1:2], common.legend = T)

ggsave(file = "supfig.jpg", fig3, bg = "transparent",
       width = 40, height = 20, units=c("cm"),
       dpi = 500, limitsize = FALSE)

xrange<-range(data$DXDTOPF)
f4a<-ggplot(data, aes(x = DXDTOPF)) + 
  geom_histogram(aes(y = ..density..), binwidth = 1, color = "black", fill = "gray", alpha = 0.5) +
  geom_density(alpha = 0.5, color = "red", size = 1) +
  labs(title = "Porcentaje de grasa total antes de transformar", x = "Valor", y = "Densidad") +
  geom_vline(xintercept=mean(data$DXDTOPF), linetype="dashed", color="red", size=1)+
  scale_x_continuous(limits = xrange, expand = c(0, 0)) +
  theme_minimal()

xrange<-range(data$o_P.TFAT)
f4b<-ggplot(data, aes(x = o_P.TFAT)) + 
  geom_histogram(aes(y = ..density..), binwidth = 0.2, color = "black", fill = "gray", alpha = 0.5) +
  geom_density(alpha = 0.5, color = "red", size = 1) +
  labs(title = "Porcentaje de grasa total despuÈs de transformar", x = "Valor", y = "Densidad") +
  geom_vline(xintercept=mean(data$o_P.TFAT), linetype="dashed", color="red", size=1)+
  scale_x_continuous(limits = xrange, expand = c(0, 0)) +
  theme_minimal()

fig4<-ggarrange(f4a, f4b, labels = LETTERS[1:2], common.legend = T)

ggsave(file = "supfig2.jpg", fig4, bg = "transparent",
       width = 40, height = 20, units=c("cm"),
       dpi = 500, limitsize = FALSE)
...
summary(fit1_f, standardized = TRUE, modindices = TRUE, fit.measures=TRUE)
interpret(fit1_f)
...
summary(fit1_m, standardized = TRUE, modindices = TRUE, fit.measures=TRUE)
interpret(fit1_m)
...
summary(fit2_f, standardized = TRUE, modindices = TRUE,  fit.measures=TRUE)
interpret(fit2_f)
...
summary(fit2_m, standardized = TRUE, modindices = TRUE, fit.measures=TRUE)
interpret(fit2_m)
...
summary(fit3_f, standardized = TRUE, modindices = TRUE, fit.measures=TRUE)
interpret(fit3_f)
...
summary(fit3_m, standardized = TRUE, modindices = TRUE, fit.measures=TRUE)
interpret(fit3_m)
